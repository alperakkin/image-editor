...
Statistics intensity(Image image)
{
    ...
    for (int y = 0; y < image.height; y++)
    {
        ...
        for (int x = 0; x < image.width; x++)
        {
            ...
            if (avg <= intensity_data.min)
                intensity_data.min = avg;
            if (avg >= intensity_data.max)
                intensity_data.max = avg;
        }
        ...
        for (int x = 0; x < image.width; x++)
        {
            ...
            for (int sy = -1; sy <= 1; sy++)
            {
                for (int sx = -1; sx <= 1; sx++)
                {
                    int posy = sy + y;
                    int posx = sx + x;
                    if (posy < 0 || posy >= image.height || posx < 0 || posx >= image.width)
                        continue;
...
int **non_maximum_suppression(int **gradient_mag, int **gradient_dir, int width, int height)
{
    ...
    for (int y = 0; y < height; y++)
    {
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            if (angle > M_PI)
                angle -= M_PI;
            if (angle >= 0 && angle < M_PI / 8)
                angle = 0;
            if (angle >= M_PI / 8 && angle < M_PI / 4)
                angle = (int)M_PI / 4;
