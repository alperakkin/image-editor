...
Image open_image(char *FILENAME)
{
  ...
  if (!fp)
    raise_error("Given path is not valid!");
  Image image;
...
Color hex_to_rgb(char *hex_code)
{
  ...
  if (hex_code == NULL || strlen(hex_code) < 7)
    raise_error("Invalid hex code.");
  ...
  {
    if (sscanf(hex_code, "%02hhx%02hhx%02hhx", &color.R, &color.G, &color.B) != 3)
      raise_error("Failed to parse hex code.\n");
  }
  if (strlen(hex_code) == 8)
  {
    if (sscanf(hex_code, "%02hhx%02hhx%02hhx%02hhx", &color.R, &color.G, &color.B, &color.A) != 4)
      raise_error("Failed to parse hex code.\n");
  }
